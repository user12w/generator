...
const bsConfirm = new bootstrap.Modal(confirm)
...
function confirmModal(callBack, data={}) {
  title.innerText = data.title || "Подтверждение";
  ...
  button.classList.add(confirmType[data.type] || "btn-primary");
    ...
    button.innerText= data.button || "Подтвердить"
  bsConfirm.show();
